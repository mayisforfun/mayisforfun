...
#define LINE_FILTER_OLD_WEIGHT 1
#define LINE_FILTER_TOTAL_WEIGHT 2
...
static int16_t scale_speed(uint16_t base_speed, uint16_t percent, uint16_t max_speed)
{
    return (int16_t) clamp_u16(((int32_t) base_speed * percent) / 100L,
                               max_speed);
}

static bool apply_table_speed(LineFollowState *state,
                              const LineFollowConfig *config,
                              uint8_t accepted_bits,
                              uint16_t base_speed)
{
    int16_t raw_error = 0;
    int16_t left_percent = 100;
    int16_t right_percent = 100;
    bool matched = true;

    switch (accepted_bits) {
    case BIT_SENSOR_C:
    ...
    state->filtered_error = raw_error;
    state->last_error = raw_error;
    if (raw_error == 0) {
        state->integral = 0;
    }

    state->left_speed = scale_speed(base_speed, (uint16_t) left_percent,
                                    config->max_speed);
    state->right_speed = scale_speed(base_speed, (uint16_t) right_percent,
                                     config->max_speed);
    return true;
}
...
void LineFollow_update(LineFollowState *state,
                       ...
                       uint16_t base_speed)
{
    /* Softer weights keep the car centered without chasing L1/R1 too hard. */
    static const int16_t weights[LINE_SENSOR_COUNT] = {-1200, -500, 0, 500, 1200};
    ...
    raw_error = state->position;

    if (apply_table_speed(state, config, accepted_bits, base_speed)) {
        return;
    }

    if ((accepted_bits == BIT_SENSOR_C) ||
        (accepted_bits == (BIT_SENSOR_L1 | BIT_SENSOR_C | BIT_SENSOR_R1)) ||
        (accepted_bits == SENSOR_MASK)) {
        raw_error = 0;
        state->filtered_error = 0;
        state->integral = 0;
    } else if (accepted_bits == (BIT_SENSOR_L1 | BIT_SENSOR_C)) {
        raw_error = -160;
    } else if (accepted_bits == (BIT_SENSOR_C | BIT_SENSOR_R1)) {
        raw_error = 160;
    } else if (accepted_bits == BIT_SENSOR_L1) {
        raw_error = -450;
    } else if (accepted_bits == BIT_SENSOR_R1) {
        raw_error = 450;
    } else if (accepted_bits == BIT_SENSOR_L2) {
        raw_error = -1100;
    } else if (accepted_bits == BIT_SENSOR_R2) {
        raw_error = 1100;
    } else if (accepted_bits == (BIT_SENSOR_L2 | BIT_SENSOR_L1)) {
        raw_error = -850;
    } else if (accepted_bits == (BIT_SENSOR_R1 | BIT_SENSOR_R2)) {
        raw_error = 850;
    }

    state->filtered_error = (int16_t)
        ((((int32_t) state->filtered_error * LINE_FILTER_OLD_WEIGHT) + raw_error) /
         LINE_FILTER_TOTAL_WEIGHT);
    ...
    if ((accepted_bits & BIT_SENSOR_C) != 0U) {
        derivative = 0;
    } else if (derivative > 500) {
        derivative = 500;
    } else if (derivative < -500) {
        derivative = -500;
    }
     ...
     */
    uint16_t outer_slow_speed = (uint16_t) (base_speed / 2U);
    uint16_t outer_fast_speed = (uint16_t) (base_speed + (base_speed / 3U));
    if (outer_fast_speed > config->max_speed) {
        outer_fast_speed = config->max_speed;
    }
